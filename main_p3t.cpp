    ...
    while(1){
       ...
       if (n_loc > 0){     //粒子が存在する場合
         ...
         for ( PS::S32 i=0; i < n_loc; i++ ){  //（粒子が存在する場合）繰り返す
           ...
           if ( N_loc > 0 ){   //存在する粒子が1個の場合（⇒相互作用エネルギーが 0）
...
//std::cout << "HELLO 1" << std::endl;
               ...
               //itr_e = 0;
               //std::cout << "●interaction energy = " << (e_now.etot - (kin_e + pot_e)) / 2 << std::endl;   //相互作用エネルギーの出力
         ...
         /*else if ( N_loc > 1 ){   //粒子が2個以上の場合 n_loc=2 （⇒相互作用エネルギーが存在する） 
             //for ( PS::S32 i=0; i<n_loc; i++ ){     //粒子吸収前の粒子情報を出力する
               //PS::ParticleSystem<FPGrav> system_grav;
//std::cout << "HELLO 2" << std::endl;
               ...
               pot_e = -(1.0 * system_grav[i].mass * FPGrav::m_sun)/(sqrt(pos*pos));    //中心BHのポテンシャルエネルギーを求める       
...
//std::cout << "HELLO 13" << std::endl;
            ...
            std::cout << "e_change = " << e_change << std::endl << std::endl;
